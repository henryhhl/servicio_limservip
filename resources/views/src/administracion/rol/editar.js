...
class EditarRol extends Component {
    ...
    get_data() {
        axios.get( web.servidor + '/rol/edit/' + this.props.match.params.id).then(
            (response) => {
                ...
                if (response.status == 200) {
                    ...
                    if (response.data.response == 1) {
                        ...
                        for (let index = 0; index < response.data.usuario_activo.length; index++) {
                            var data = response.data.usuario_activo[index];
                            this.state.target_usuario.push(data.id.toString());
    ...
    onSubmit() {
        ...
        // formdata.append('array_permiso', JSON.stringify(this.state.permisos));
    ...
    onChecked(data, index) {
        if (typeof this.state.permisos[index] == 'undefined') {
            var objeto = {
                id: data.id,
                checked: (data.estado == 'A')?true:false,
                estado: (data.estado == 'A')?'1':'0',
            };
            this.state.permisos[index] = objeto;
        }
        return this.state.permisos[index].checked;
    }
    onChangePermiso(pos) {
        this.state.permisos[pos].checked = !this.state.permisos[pos].checked;
        this.state.permisos[pos].estado = this.state.permisos[pos].checked?'1':'0';
