...
class IndexPersonal extends Component {
    ...
    componentDidMount() {
        this.props.get_link('personal', true); //#3f6ad8 co //#e0f3ff bg bold
        this.get_data();
    }
    get_data(page = 1, search = '') {
        axios.get( web.servidor + '/personal/index?page=' + page + '&search=' + search).then(
            (response) => {
                ...
                if (response.status == 200) {
                    ...
                    if (response.data.response == 1) {
                        this.props.getpersonal(response.data.data.data, response.data.pagination, page, '');
                        return;
                    }
                }
                Modal.error({
    ...
    onAdd() {
        this.props.history.push( web.serv_link + '/personal/create');
    }
    formato(date) {
    ...
    onShow(data, event) {
        ...
        this.props.history.push( web.serv_link + '/personal/show/' + data.id);
    }
    onEdit(data, event) {
        event.preventDefault();
        this.props.history.push( web.serv_link + '/personal/editar/' + data.id);
    }
    onDelete(data) {
        this.props.onModalActive(data, 'personal');
    }
    onChangePagina(page) {
                ...
                <div className="cards">
                    <div className="card-header-tab card-header mt-4" style={{border: '1px solid transparent'}}>
                        ...
                        <div className="btn-actions-pane-right text-capitalize mb-4">
                            { isPermission(this.props.permisos_habilitados, permissions.personalcreate) ?
                    ...
                    <div className="forms-groups">
                                    ...
                                    {this.props.personal.map(
                                        (data, key) => (
                                            <tr key={key}>
                                                ...
                                                <td>
                                                    <label className='cols_show'>Ci: </label>
                                                        {data.ci} &nbsp;
                                                </td>
                                                <td>
                                                ...
                                                { isPermission(this.props.permisos_habilitados, permissions.personaleditar) ?
                                                ...
                                                { isPermission(this.props.permisos_habilitados, permissions.personaldelete) ?
                        ...
                        <div className="cols-lg-12 cols-md-12 cols-sm-12 cols-xs-12 main-card card card-body">
                            <Pagination 
                                total={this.props.pagination.personal.total}
                                current={this.props.paginate.personal}
                                pageSize={10}
...
export default withRouter(IndexPersonal);
