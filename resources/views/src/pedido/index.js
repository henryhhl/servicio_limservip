...
class IndexMySolicitud extends Component {
    ...
    componentDidMount() {
        this.props.get_link('mysolicitud_pedido', true); //#3f6ad8 co //#e0f3ff bg bold
        this.get_data();
    }
    get_data(page = 1, search = '') {
        axios.get( web.servidor + '/mysolicitud_pedido/index?page=' + page + '&search=' + search).then(
            (response) => {
    ...
    onAdd() {
        this.props.history.push( web.serv_link + '/mysolicitud_pedido/create');
    }
    formato(date) {
    ...
    onShow(data, event) {
        ...
        this.props.history.push( web.serv_link + '/mysolicitud_pedido/show/' + data.id);
    }
    onDelete(data) {
...
export default withRouter(IndexMySolicitud);
