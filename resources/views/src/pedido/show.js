...
class ShowMySolicitud extends Component {
    ...
    componentDidMount() {
        this.props.get_link('mysolicitud_pedido', true);
        this.get_data();
    }
    get_data() {
        axios.get( web.servidor + '/mysolicitud_pedido/show/' + this.props.match.params.id ).then(
            (response) => {
    ...
    onUpdateEstado() {
        if (this.state.proceso == 'P') {
            notification.warning({
                description: 'FAVOR DE SELECCIONAR EL ESTADO YA QUE EL ACTUAL ESTA EN PENDIENTE.',
            });
        }else {
            this.onSesion();
        }
            ...
            <div className="rows">
                ...
                <div className="cards">
                    <div className='forms-groups'>
                        <Card
                            style={{ width: '100%', minWidth: '100%', }}
                            title="DETALLE SOLICITUD PEDIDO"
...
ShowMySolicitud.propTypes = {
    buttoncolor: PropTypes.string,
}

ShowMySolicitud.defaultProps = {
    buttoncolor: '',
}

export default withRouter(ShowMySolicitud);
